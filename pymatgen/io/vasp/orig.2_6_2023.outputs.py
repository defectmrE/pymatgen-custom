...
def plot_defect_level_diagrams(eigenval, procar, vbm, cbm, align_corr=0,
    ...
    for spin, egv in int_egv.items():
        ...
        plt_ind = np.argwhere((egv[:, 0] < cbm + Emax - 0.2) & (egv[:, 0] > vbm + Emin +0.2))
    ...
    #norm = matplotlib.colors.Normalize(vmin=0, vmax= max(max(localizer[Spin.up][plt_range[Spin.up]]),max(localizer[Spin.down][plt_range[Spin.down]])) * 1.5)
...
def plot_defect_TM_d(eigenval, procar, defect_site_idx, vbm, cbm, align_corr=0,
                               label_degeneracy=False, tol=5e-3, Emin=-0.6, Emax=0.3, arrow_length_factor=0.1,
                               title="", integerize = False, input_netspin=None,norm=True,orb='d'):
    """
    Args:
        eigenval: Eigenval object
        procar: Procar object
    ...
    def plot_state_coords(N):
        line_range = np.linspace(0.25 - (N / 2 - 1) * 0.05, 0.35 + (N / 2 - 1) * 0.05, N)
        line_start = [i - 0.02 for i in line_range]
        line_end = [i + 0.02 for i in line_range]
        return iter(zip(line_start, line_end))
    ...
    if integerize:
        ...
        int_egv, net_spin = eigenval.get_integer_occupancy_eigenvalues(align_corr)
        localizer = procar.localize_d_quantizer(defect_site_idx,norm=norm,orb=orb)
    ...
    if label_degeneracy:
        for spin, data in int_egv.items():
            ...
            for step, idx in enumerate(plt_range[spin]):
                ...
                e, occ = e_occ
                e = e - vbm
                ax.hlines(e, line_start, line_end, color=cmap(localizer[spin][idx]), linewidth=1)

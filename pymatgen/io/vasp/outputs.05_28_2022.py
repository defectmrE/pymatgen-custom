...
##added
    ...
    def localize_quantizer(self, defect_site_ind):
        ...
        :param spin: Spin
        :param defect_site: Periodic Site object (or a list of periodic site obkect)
...
def plot_defect_level_diagrams(eigenval, procar, defect_site_idx, vbm, cbm, align_corr=0, vbm_to_zero=True,
                               plot_degeneracy=True, tol=1e-3, Emin=-0.6, Emax=0.3, title=""):
    """
    Args:
    ...
    for spin, data in int_egv.items():
        ...
        for step, idx in enumerate(plt_range[spin]):
            ...
            e, occ = e_occ
            e = e - vbm
            ax.hlines(e, line_start, line_end, color=cmap(localizer[spin][idx]), linewidth=2)
            if occ > 1e-8:
                ax.arrow(arrow_start + step * arrow_step, e + arrow_tail,
                         0, arrow_head, width=0.01, ec='green', color='green')

            # add degenerate label
            if plot_degeneracy:
                if degenerate_label[spin][idx] > 1:
                    ax.text(line_end + 0.1, e, str(degenerate_label[spin][idx]), ha='center', va='center')

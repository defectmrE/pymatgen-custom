...
def plot_defect_level_diagrams(eigenval, procar, defect_site_idx, vbm, cbm, align_corr=0, vbm_to_zero=True,
                               plot_degeneracy=True, tol=5e-3, Emin=-0.6, Emax=0.3, title="", integerize = False,
                               input_netspin=None):
    """
    Args:
        ...
        vbm_to_zero: Align VBM to 0, default to True
    ...
    for spin, egv in int_egv.items():
        ...
        plt_ind = np.argwhere((egv[:, 0] < cbm) & (egv[:, 0] > vbm - 0.1))
    ...
    for spin, data in int_egv.items():
        ...
        if spin is Spin.up:
            ...
            line_end = 0.6
            arrow_start = 0.25
            arrow_tail = (cbm - vbm) * 0.2*(-1/3)
            arrow_head = (cbm - vbm) * 0.2*(2/3)
        else:
            line_start = 0.8
            line_end = 1.2
            arrow_start = 0.85
        ...
        for step, idx in enumerate(plt_range[spin]):
            ...
            if occ > 1e-8:
                ax.arrow(arrow_start + step * arrow_step, e + arrow_tail,
                         0, arrow_head, width=0.001, ec='green', color='green',head_width=0.01)

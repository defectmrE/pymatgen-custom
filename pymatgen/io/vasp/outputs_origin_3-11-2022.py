...
def plot_defect_level_diagrams(eigenval, procar, vbm, cbm, defect_site_idx=None, align_corr=0,
                               label_degeneracy=False, tol=5e-3, Emin=-0.6, Emax=0.3, arrow_length_factor=0.1,
                               title="", integerize = False, input_netspin=None):
    """
    Args:
    ...
    if integerize:
        ...
        localizer = procar.ipr(defect_site_ind=defect_site_idx)
    ...
    for spin, egv in int_egv.items():
        ...
        #print(plt_ind)
        plt_ind = np.arange(min(plt_ind), max(plt_ind))
    ...
    #sm = plt.cm.ScalarMappable(cmap=cmap, norm=matplotlib.colors.Normalize(vmin=0, vmax=0.1))
    ...
    #print(max(max(localizer[Spin.up][plt_range[Spin.up]]),max(localizer[Spin.down][plt_range[Spin.down]])))

...
from atomate.vasp.analysis.vaspwfc import get_all_tdm_and_lifetime
...
class Vasprun(MSONable):
    ...
    **Vasp inputs**
    ...
    def __init__(self, filename, ionic_step_skip=None,
        ...
        if self.incar.get("ALGO", "") not in ["CHI", "BSE", "Eigenval"] and (not self.converged):
...
class Procar:
        ...
        with zopen(filename, "rt") as f:
            ...
            for l in f:
                ...
                if bandexpr.match(l):
                    ...
                    def f3():
                        return np.zeros((nkpoints, nbands, nions + 1, 1)) # 1 extra header to include total contribution
                    ...
                    tot_num_data = np.array([float(toks[-1])])# adding total number data, which was poped
                    # print(num_data,"zz")
                    if not done:
                        data[spin][current_kpoint, current_band,
                        index, :] = num_data
                        # print("oj")
                        tot[spin][current_kpoint, current_band, index] = tot_num_data

                    else:
                        ##insert_23_12_2020
                        if len(toks) > len(headers):
                            # print("hi",toks, headers)
                    ...
                    tot_contri = np.array([float(l.split()[-1])])
                    tot[spin][current_kpoint, current_band, index+1] = tot_contri
                    done = True
    ...
    def localize_quantizer(self, defect_site_ind):
        # TODO: find defect site in the super structure
        # TODO: Remove vacancy sites
        """
        Localization of states are determined using PROCAR, the contribution of specific wavefunction
        is determined by |\langle Y_{{lm}}^{{\alpha }}|\phi _{{n{\mathbf  {k}}}}\rangle |^{2}, represents
        the wave function on the spherical harmonics centered on ion alpha. Currently only support the
        total projection.

        :param procar: pymatgen Procar object
        :param structure: pymatgen Structure object (generated defective supercell)
        :param spin: Spin
        :param defect_site: Periodic Site object (or a list of periodic site object)

        """
        loc_quant_dict = {}

        nkpts = self.nkpoints if self.nkpoints else 1
        weights = self.weights if self.weights else np.array([1.])

        if nkpts != 1:
            raise ValueError("Only implemented for Gamma calculation!")

        """
        for spin, d in self.data.items():
            loc_quant_list = []

            for kpt_ind in range(nkpts):
                # consider all bands and sum all orbitals and gives shape of (bands * natoms)
                total_contri = np.sum(d[kpt_ind], axis=2)
                defect_contri = np.sum(total_contri[:, defect_site_ind], axis=1)
                loc_quant_list.append(
                    defect_contri / np.sum(total_contri, axis=1) * weights[kpt_ind]
            )
            loc_quant = np.sum(loc_quant_list, axis=0)
            loc_quant_dict[spin] = loc_quant
        """

        for spin, d in self.total_contribution.items():
            loc_quant_list = []

            for kpt_ind in range(nkpts):
                total_contri = d[kpt_ind][:,-1,:]

                defect_contri = d[kpt_ind][:,:-1,:]
                defect_contri = defect_contri.reshape(defect_contri.shape[:2])
                defect_contri = defect_contri/total_contri

                loc_quant_dict[spin] = np.sum(defect_contri[:, defect_site_ind], axis=1)

        return loc_quant_dict

    def ipr(self, kpt_ind=0, defect_site_ind=None):
        """
        Inverse participation ratio is used to measure the localization of states are determined using PROCAR

        ipr(E_i) = \sum_n \rho^2_n(E_i) / ( \sum_n \rho_n(E_i) )^2
        """
        ipr_dict = {}
        ...
        for spin, d in self.total_contribution.items():
            ...
            total_contri = d[kpt_ind][:,-1,:]
            total_contri = total_contri.reshape(total_contri.shape[0])

            defect_contri = d[kpt_ind][:, :-1, :]

            if defect_site_ind is not None: # site projected IPR
                defect_contri = defect_contri[:, defect_site_ind, :]

            defect_contri = defect_contri.reshape(defect_contri.shape[:2])

            ipr_dict[spin] = np.sum(defect_contri**2, axis=1) / total_contri**2

        return ipr_dict

    def localize_d_quantizer(self, defect_site_ind, norm = True, orb=['tot']):
        # TODO: find defect site in the super structure
        # TODO: Remove vacancy sites
        """
        Localization of states are determined using PROCAR, the contribution of specific wavefunction
        ...
        :param spin: Spin
        :param defect_site: Periodic Site object (or a list of periodic site object)
        :param norm: normalize the d orbital contribution to 1
        :param orb: dxy, dyz, dz2, dxz, dx2-y2: 4, 5, 6, 7, 8
                    D3h: Z2, x2-y2 + xy, yz + xz
                    C3v: xz,yz,xy,x2-y2; z2
        

        """
        d_orb_quant_dict = {}

        d_orb_dict = {'xy':4, 'yz':5, 'z2':6, 'xz':7, 'x2-y2':8}
        ...
        if orb == ['tot']:
            orb_list = np.array([4,5,6,7,8])
        else:
            try:
                orb_list = np.array([d_orb_dict[i] for i in orb])
            except KeyError:
                raise ValueError("Only xy, yz, z2, xz, x2-y2 are valid options!")

        for spin, d in self.data.items():
            for kpt_ind in range(nkpts):
                d_contri = d[kpt_ind][:,:,orb_list]
                d_orb_quant_dict[spin] = np.sum(d_contri, axis=2)
        ...
        if norm:
            for spin, d in self.total_contribution.items():
                ...
                d_orb_quant_dict[spin] /= total_contri

        for spin, d in d_orb_quant_dict.items():
            d_orb_quant_dict[spin] = d[:, defect_site_ind]

        return d_orb_quant_dict
...
class Wavecar:
    ...
    def prob_density(self, kpoint, band, spin):
        psi = np.fft.fftn(
            self.fft_mesh(kpoint, band, spin=spin, shift=False))  # TODO: should this always be shift False
...
def parse_qis_data(wavecar_path, procar, eigenval_raw, ref_ind=3.88, kpoints=1,
                   gam=False, vbm=5.6158, cbm=6.2268, all_combination=True):

    tdm_entry = {}
    procar_entry = {}

    tdm_entry_raw = get_all_tdm_and_lifetime(ref_ind, kpoints, wavecar_path, gam,
                                             eigenval_raw, vbm=vbm, cbm=cbm, all_combination=all_combination)

    for spin in tdm_entry_raw:
        tdm_entry[str(spin)] = tdm_entry_raw[spin]

    procar_entry_raw = procar.ipr()
    for spin in procar_entry_raw:
        procar_entry[str(spin)] = procar_entry_raw[spin]

    return {"tdm": tdm_entry, "ipr": procar_entry}
...
class Eigenval:
    ...
    def get_integer_occupancy_eigenvalues(self, corr=0, kpt_ind=0):
        """
        :param corr: potential alignment correction terms for Kohn-Sham states, default=0

        :return: eigenvalues and intergerized occupancy and spin states as a tuple
        """
        #if self.nkpt != 1:
        #    raise ValueError("Only works for Gamma point calculation for now.")

        egv_dict = {}
        egv_copy = copy.deepcopy(self.eigenvalues)

        for spin, data in egv_copy.items():  # remove the k-point dimension
            d = data[kpt_ind]
            d[:, 0] += corr
            egv_dict[spin] = d

        if len(egv_dict.keys()) == 2:  # spin-polarized calculations
            par_occ_dict = {}

            for spin, data in egv_dict.items():
                tmp_part_dict = {}
                par_occ_band_ind = []
                for band_ind, egv_occ in enumerate(data):
                    egv, occ = egv_occ
                    if 1e-8 < occ < 1 - 1e-8:
                        par_occ_band_ind.append(band_ind)

                if par_occ_band_ind:
                    tmp_part_dict["part_band_ind"] = par_occ_band_ind
                    tmp_part_dict["num_nelect"] = np.sum(data[:, 1][par_occ_band_ind])
                    par_occ_dict[spin] = tmp_part_dict

            nelect_to_dist = 0
            for spin, data in par_occ_dict.items():
                nelect_to_dist += data["num_nelect"]
            nelect_to_dist = int(np.round(nelect_to_dist))

            if len(par_occ_dict) == 2:
                fill_spin_up = par_occ_dict[Spin.up]['part_band_ind'][0]
                fill_spin_dw = par_occ_dict[Spin.down]['part_band_ind'][0]

                # TODO: might not work for certain types of constraint DFT? eg, partial occupancy in VBM (rare though)
                while nelect_to_dist > 0:
                    if egv_dict[Spin.up][fill_spin_up][0] \
                            <= egv_dict[Spin.down][fill_spin_dw][0]:
                ...
                egv_dict[Spin.up][:fill_spin_up][:, 1] = 1
                egv_dict[Spin.up][fill_spin_up:][:, 1] = 0
                egv_dict[Spin.down][:fill_spin_dw][:, 1] = 1
                egv_dict[Spin.down][fill_spin_dw:][:, 1] = 0

            elif len(par_occ_dict) == 1:
                for spin, data in par_occ_dict.items():
                    fill_nelect = data['part_band_ind'][nelect_to_dist - 1]
                    egv_dict[spin][:fill_nelect + 1][:, 1] = 1
                    egv_dict[spin][fill_nelect + 1:][:, 1] = 0

            # return spin state S = 0.5
            net_spin = 0.5 * (np.sum(egv_dict[Spin.up][:, 1])
                            - np.sum(egv_dict[Spin.down][:, 1]))
        else:
            net_spin = 0

        return egv_dict, net_spin
...
def plot_defect_level_diagrams(eigenval, procar, vbm, cbm, align_corr=0, kpt_ind = 0, defect_site_ind=None,
                               label_degeneracy=False, tol=5e-3, Emin=-0.6, Emax=0.3, arrow_length_factor=0.1,
                               title="", integerize = False, input_netspin=None, excitation_dict={},filter_dict={}):
    """
    Args:
        eigenval: Eigenval object
        procar: Procar object
        defect_site_idx: list of atom index for the defect site, use for plotting localization
        vbm: bulk vbm (eV)
        cbm: bulk cbm (eV)
        align_corr: Alignment corrections for Kohn-Sham levels
        plot_degeneracy: plot the degeneracy number next to the states
        tol: degeneracy tolerance, default to 0.01 eV
        Emin: plot lowerbound, VBM + Emin
        Emax: plot upperbound, VBM - Emax
        title: Name of the plot

    Returns: defect diagram plot

    """
    def degeneracy(A, atol):
        runs = np.split(A, np.where(np.abs(np.diff(A)) >= atol)[0] + 1)
        return runs


    def plot_state_coords(N,ext=0):
        line_range = np.linspace(0.25 - (N / 2 - 1) * 0.1 - (N-1)*ext, 0.35 + (N / 2 - 1) * 0.1 + (N-1)*ext, N)
        line_start = [i - 0.04-ext for i in line_range]
        line_end = [i + 0.04+ext for i in line_range]
        return iter(zip(line_start, line_end))

    if integerize:
        if input_netspin is None:
            raise ValueError("Using intergerized PROCAR and EIGENVAL requires the input of net spin!")
        else:
            int_egv = copy.deepcopy(eigenval)
            localizer = copy.deepcopy(procar)
            net_spin = input_netspin
    else:
        int_egv, net_spin = eigenval.get_integer_occupancy_eigenvalues(corr=align_corr, kpt_ind=kpt_ind)
        if defect_site_ind is not None:
            localizer = procar.ipr(kpt_ind=kpt_ind, defect_site_ind=defect_site_ind)
        else:
            localizer = procar.ipr()

    for spin in int_egv:
        if not isinstance(spin, Spin):
            int_egv[Spin(int(spin))] = int_egv.pop(spin)
    for spin in localizer:
        if not isinstance(spin, Spin):
            localizer[Spin(int(spin))] = localizer.pop(spin)

    plt_range = {}

    for spin, egv in int_egv.items():
        # TODO: add the while function for finding finite number of states for plotting
        #plt_ind = np.argwhere((egv[:, 0] < cbm + Emax - 0.3) & (egv[:, 0] > vbm + Emin +0.3))
        plt_ind = np.argwhere((egv[:, 0] < cbm + Emax) & (egv[:, 0] > vbm + Emin))
        plt_ind = plt_ind.reshape(plt_ind.shape[0])
        #plt_ind = np.arange(min(plt_ind) - 3, max(plt_ind) + 6)

        plt_range[spin] = plt_ind

    ext = int(sum(int_egv[Spin.up][plt_range[Spin.up]][:, 1]) -
              sum(int_egv[Spin.down][plt_range[Spin.down]][:, 1]) - net_spin)

    if ext > 0:  # one extra spin.down
        plt_range[Spin.down] = np.arange(min(plt_range[Spin.down]) - ext, max(plt_range[Spin.down]) + 1)
    elif ext < 0:  # one extra spin.up
        plt_range[Spin.up] = np.arange(min(plt_range[Spin.up]) + ext, max(plt_range[Spin.up]) + 1)

    # create the degeneracy index
    degenerate_label = {}
    for spin, data in int_egv.items():
        degenerate_ind = []
        egv = data[:, 0]
        degenerate_ind_gp = degeneracy(egv, tol)
        tmp_interval_ind = None

        for e in egv:
            found = False
            for interval_ind, interval in enumerate(degenerate_ind_gp):
                if e == interval[0]:
                    if interval_ind != tmp_interval_ind:
                        tmp_interval_ind = interval_ind
                        found = True
                        break
            if found:
                degenerate_ind.append(len(interval))
            else:
                degenerate_ind.append(0)
        degenerate_label[spin] = degenerate_ind
    ...
    ax.set_title(title + f" S={net_spin}")

    if label_degeneracy:
        for spin, data in int_egv.items():

            if len([i for i in int_egv[spin][plt_range[spin]] if i[1]]) == 0:
                arrow_step = 0
            else:
                arrow_step = 0.3 / len([i for i in int_egv[spin][plt_range[spin]] if i[1]])

            if spin is Spin.up:
                line_start = 0.2
                line_end = 0.6
                arrow_start = 0.25
                arrow_tail = (cbm - vbm + Emax - Emin) * arrow_length_factor * (-1/3)
                arrow_head = (cbm - vbm + Emax - Emin) * arrow_length_factor * (2/3)
            else:
                line_start = 0.8
                line_end = 1.2
                arrow_start = 0.85
            ...
            for step, idx in enumerate(plt_range[spin]):
                e_occ = data[idx]
                e, occ = e_occ
                e = e - vbm
                ax.hlines(e, line_start, line_end, color=cmap(norm(localizer[spin][idx])), linewidth=1)

                if occ > 1e-8:
                    ax.annotate('', xy=(arrow_start + step * arrow_step, e + arrow_tail),
                                    xytext=(arrow_start + step * arrow_step, e + arrow_head),
                                arrowprops=dict(arrowstyle='<-', color='black', lw=1, ls='-',),
                                annotation_clip=True
                                )

                # add degenerate label
                if degenerate_label[spin][idx] > 1:
                    ax.text(line_end + 0.1, e, str(degenerate_label[spin][idx]), ha='center', va='center',  clip_on=True)

    else:
        excitation_arrow = [None, None]
        for spin, data in int_egv.items():
            ...
            for step, idx in enumerate(plt_range[spin]):
                ...
                if excitation_dict and filter_dict:
                    mid_start, mid_end = next(plot_state_coords(1,ext=0.02))
                    if str(spin) == excitation_dict['spin']:
                        if excitation_dict['initial_band'] == idx:
                            excitation_arrow[0] = [(mid_start + mid_end) / 2 + hori_shift, e]
                        if excitation_dict['final_band'] == idx:
                            excitation_arrow[1] = [(mid_start + mid_end) / 2 + hori_shift, e]

                # depends on the filter dict, we needs to take care of the excitation dict...
                if filter_dict:
                    ipr_thr = filter_dict.get('ipr_thr', 0.005) # 0.005 is the default value for the ipr_thr
                    in_gap_thr = filter_dict.get('in_gap_thr', 0.1) # +- 100 meV with respect to VBM
                    
                    # add a plot flag rather than ...
                    if (localizer[spin][idx] < ipr_thr) and ((e < vbm + in_gap_thr) or (e > cbm - in_gap_thr)):
                        # plot the delcalized level...
                        if (str(spin) == excitation_dict['spin']) and ((excitation_dict['initial_band'] == idx) or (excitation_dict['final_band'] == idx)):
                        # plot the delocalized levels...
                            ax.hlines(e, mid_start+hori_shift, mid_end+hori_shift, color='black', linewidth=3)
                            if occ > 1e-8:
                                ann = ax.annotate('', xy=( (mid_start + mid_end)/2 + hori_shift, e + arrow_tail),
                                            xytext=( (mid_start + mid_end)/2 + hori_shift, e + arrow_head),
                                            arrowprops=dict(arrowstyle='<-', color='black', lw=1, ls='-'),
                ...
                # solve the initial boundary case
                current_degen_states=0
                if step == 0 and degenerate_label[spin][idx] == 0:
                    while degenerate_label[spin][idx+current_degen_states] == 0:
                        current_degen_states += 1
                    degenerate_label[spin][idx] = current_degen_states

                # check for the final boundary case
                if degenerate_label[spin][idx] > 0:
                    if np.count_nonzero(degenerate_label[spin][idx:plt_range[spin][-1]+1]) == 1:
                        degenerate_label[spin][idx] = len(degenerate_label[spin][idx:plt_range[spin][-1]+1])

                if degenerate_label[spin][idx] > 0: # this will tell if this is a degenearte so we should keep using the plot_coords
                    if filter_dict:
                        plot_coords = plot_state_coords(degenerate_label[spin][idx],ext=0.02)
                    else:
                        plot_coords = plot_state_coords(degenerate_label[spin][idx])

                try:
                    line_start, line_end = next(plot_coords)
                except StopIteration:
                    continue

                line_start += hori_shift
                line_end += hori_shift

                if filter_dict:
                    ax.hlines(e, line_start, line_end, color=cmap(norm(localizer[spin][idx])), linewidth=3)
                else:
                    ax.hlines(e, line_start, line_end, color=cmap(norm(localizer[spin][idx])), linewidth=1)
                if occ > 1e-8:
                    ann = ax.annotate('', xy=( (line_start+line_end)/2, e + arrow_tail),
                                xytext=( (line_start+line_end)/2, e + arrow_head),
                                arrowprops=dict(arrowstyle='<-', color='black', lw=1, ls='-'),
                                annotation_clip=False)
                    ann.arrow_patch.set_clip_box(ax.bbox)
                
                if excitation_dict and not filter_dict:
                    if str(spin) == excitation_dict['spin']:
                        if excitation_dict['initial_band'] == idx:
                            excitation_arrow[0] = [(line_start + line_end) / 2, e]
                        if excitation_dict['final_band'] == idx:
                            excitation_arrow[1] = [(line_start + line_end) / 2, e]

    if excitation_arrow[0] and excitation_arrow[1]:
        ax.annotate('', xy=(excitation_arrow[1][0], excitation_arrow[1][1]),
                    xytext=(excitation_arrow[0][0], excitation_arrow[0][1]),
                    arrowprops=dict(arrowstyle='simple', color='orange', alpha=1),
                    annotation_clip=True, zorder=-1)

        text_coords = [(excitation_arrow[1][0] + excitation_arrow[0][0])/2,
                    (excitation_arrow[1][1]+excitation_arrow[0][1])*0.4]

        try:
            ax.text(text_coords[0], text_coords[1], rf"$\Delta$KS={round(excitation_dict['hse0_ks_diff'],2)} eV"
                                                    "\n"
                                                    rf"$\mu$={round(excitation_dict['mu'],2)}D")

        except KeyError:
            ax.text(text_coords[0], text_coords[1], rf"$\Delta$KS={round(excitation_dict['ks_diff'],2)} eV"
                                                    "\n"
                                        rf"$\mu$={round(excitation_dict['mu'],2)}D")


    vbm_rect = patches.Rectangle((0, Emin), 1.4, 0-Emin, linewidth=0.1, edgecolor='none',
                                 facecolor='#B3BFD0', zorder=-10)
    cbm_rect = patches.Rectangle((0, cbm-vbm), 1.4, Emax, linewidth=0.1, edgecolor='none',
                                 facecolor='#ACC2A2', zorder=-10)
    ...
    plt.ylim(Emin, cbm-vbm+Emax)
    plt.xlim(0, 1.3)
    sm.set_array([])
    cbar = plt.colorbar(sm)
    cbar.set_label('Inverse participation ratio', rotation=270, labelpad=10)
    return fig

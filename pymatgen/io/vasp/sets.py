...
class OrigMPHSE0StaticSet(MPHSERelaxSet):
    """
    Run a HSE0 calculation using MPHSERelaxSet, in the similar fasion of G0W0, a single
    shot of HSE to re-calculate the eigenvalues.
    """
    ...
    def __init__(self, structure, **kwargs):
        ...
        self._config_dict["INCAR"].update(hse0_incar)
        self.kwargs = kwargs

    @classmethod
    def from_prev_calc(cls, prev_calc_dir, copy_wavecar=True, copy_chgcar=True, **kwargs):
        """
        ...
        Args:
            ...
            copy_chgcar (bool): whether to copy WAVECAR of previous run
            \\*\\*kwargs: All kwargs supported by MPHSE0StaticSet,
        ...
        prev_structure = get_structure_from_prev_run(vasprun, outcar,
                                                     sym_prec=0)
        if "NELECT" in vasprun.incar:
            potcar = sorted(glob.glob(str(Path(prev_calc_dir) / "POTCAR*")))[0]
            potcar = Potcar.from_file(potcar)

            site_symbols = list(set(prev_structure.symbol_set))
            nelect = 0.
            for ps in potcar:
                if ps.element in site_symbols:
                    site_symbols.remove(ps.element)
                    nelect += prev_structure.composition.element_composition[
                                  ps.element] * ps.ZVAL

            charge = nelect - vasprun.incar['NELECT']
            prev_structure.set_charge(charge)

        files_to_transfer = {}

        if copy_wavecar:
            wavecars = sorted(glob.glob(str(Path(prev_calc_dir) / "WAVECAR*")))
            if wavecars:
                files_to_transfer["WAVECAR"] = str(wavecars[-1])
...
class MPHSE0StaticSet(MPHSERelaxSet):
    ...
    def incar(self):
        ...
        # remove LVTOT and LVHAR
        incar.pop('LVHAR', None)
        incar.pop('LVTOT', None)
        # remove LDAU
        incar.pop('LDAU', None)
        incar.pop('LDAUU', None)
        incar.pop('LDAUJ', None)
...
class MPSOCSet(MPStaticSet):
    ...
    def from_prev_calc(cls, prev_calc_dir, copy_chgcar=True,
        ...
        if "magmom" in kwargs: # ensure it is removed
            kwargs.pop("magmom", None)

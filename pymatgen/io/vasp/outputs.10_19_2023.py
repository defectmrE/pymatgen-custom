...
class Procar:
    ...
    def localize_d_quantizer(self, defect_site_ind, norm = True, orb='d'):
        # TODO: find defect site in the super structure
        # TODO: Remove vacancy sites
        """
        Localization of states are determined using PROCAR, the contribution of specific wavefunction
        ...
        for spin, d in self.data.items():
            for kpt_ind in range(nkpts):
                if orb== 'd':
                    d_contri = d[kpt_ind][:,:,4:]
                elif orb == 'e':
                    orb_idx = np.array([6,8])
                    d_contri = d[kpt_ind][:,:,orb_idx]
                elif orb == 't2':
                    orb_idx = np.array([4,5,7])
                    d_contri = d[kpt_ind][:, :, orb_idx]
                else:
                    raise ValueError("Only d, e, and t2 are valid options!")
...
class Wavecar:
    ...
    def _generate_G_points(self, kpoint: np.ndarray, gamma: bool = False):
        """
...
def plot_defect_level_diagrams(eigenval, procar, vbm, cbm, align_corr=0, kpt_ind=0,
                               label_degeneracy=False, tol=5e-3, Emin=-0.6, Emax=0.3, arrow_length_factor=0.1,
                               title="", integerize = False, input_netspin=None, excitation_dict={},defect_site_ind=None):
    """
    Args:
    ...
    def plot_state_coords(N):
        line_range = np.linspace(0.25 - (N / 2 - 1) * 0.1, 0.35 + (N / 2 - 1) * 0.1, N)
        line_start = [i - 0.04 for i in line_range]
        line_end = [i + 0.04 for i in line_range]
    ...
    if integerize:
        ...
        if defect_site_ind is not None:
            #TODO: assert type
            localizer = procar.ipr(defect_site_ind=defect_site_ind, kpt_ind=kpt_ind)
        else:
            localizer = procar.ipr(kpt_ind=kpt_ind)

    for spin in int_egv:
        if not isinstance(spin, Spin):
            int_egv[Spin(int(spin))] = np.asarray(int_egv.pop(spin))
    for spin in localizer:
        if not isinstance(spin, Spin):
            localizer[Spin(int(spin))] = np.asarray(localizer.pop(spin))
    ...
    if label_degeneracy:
        ...
        for spin, data in int_egv.items():
            ...
            for step, idx in enumerate(plt_range[spin]):
                #print(f"{spin}:{localizer[spin][idx]}")
                e, occ = data[idx]
                e = e - vbm
                ...
                if occ > 1e-8:
                    ax.annotate('', xy=( (line_start+line_end)/2, e + arrow_tail),

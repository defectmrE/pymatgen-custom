...
class DosPlotter:
    ...
    def save_plot(self, filename, img_format="pdf", xlim=None, ylim=None):
        """
    ...
    def show(self, xlim=None, ylim=None):
        ...
        plt = self.get_plot(xlim, ylim)
        plt.show()
    def get_plot_flipped(self, xlim=None, ylim=None, gap=0):
        """
        Get a matplotlib plot showing the DOS.
        ...
        for i, key in enumerate(keys):
            ...
            allpts.extend(list(zip(x, y)))
            if self.stack:
                plt.fill(y, x, color=colors[i % ncolors],
                         label=str(key))
            else:
        ...
        if self.zero_at_efermi:
            xlim = plt.xlim()
            #plt.plot([0, 0], xlim, 'k--', linewidth=2)
            plt.axhline(0,color='r',ls='--')
            plt.axhline(gap,color='red',ls='--')
    ...
    def save_plot_flipped(self, filename, img_format="pdf", xlim=None, ylim=None,gap=0):
        """
        Save matplotlib plot to a file.
        ...
        Args:
            ...
            ylim: Specifies the y-axis limits.
        """
        plt = self.get_plot_flipped(xlim, ylim,gap)
        plt.savefig(filename, format=img_format)
...
class CohpPlotter:
    ...
    def get_plot(self, xlim=None, ylim=None, plot_negative=None,
                 integrated=False, invert_axes=True,gap=0):
        """
        ...
        if not invert_axes:
            ...
            if self.zero_at_efermi:
                print(self._cohps[key]['efermi'])
                plt.plot([0, 0], ylim, "r--", linewidth=2)
                plt.axhline(gap,color='r',ls='--')
            else:
            ...
            print(self._cohps[key]['efermi'])
            if self.zero_at_efermi:
                plt.plot(xlim, [0, 0], "r--", linewidth=2)
                plt.axhline(gap,color="r", ls='--')
            else:
    ...
    def save_plot(self, filename, img_format="pdf", xlim=None, ylim=None,gap=0):
        """
        Save matplotlib plot to a file.
        ...
        Args:
            ...
            ylim: Specifies the y-axis limits. Defaults to None for
                automatic determination.
        """
        plt = self.get_plot(xlim, ylim,gap)
        #print(self._cohps[key]['efermi'])
        plt.savefig(filename, format=img_format)

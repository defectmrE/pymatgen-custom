...
from pymatgen.analysis.defects.core import Interstitial, \
    Defect, Vacancy, Substitution, ComplexMV, ComplexMI
...
class PointDefectComparator(MSONable):
    ...
    def are_equal(self, d1, d2):
        ...
        """
        possible_defect_types = (Defect, Vacancy, Substitution, Interstitial, ComplexMV, ComplexMI)
        complex_defect_types = (ComplexMV, ComplexMI)
        complex_defect = False
        ...
        if not isinstance(d1, complex_defect_types):
            if d1.site.specie != d2.site.specie:
                return False
            elif self.check_charge and (d1.charge != d2.charge):
                return False

        if isinstance(d1, complex_defect_types): # complex defect
            complex_defect = True
            #d1_specie_list = [sp.specie for sp in d1.site]
            #d2_specie_list = [sp.specie for sp in d2.site]
            d1_specie_list = [ sp.split(']_')[-1] if isinstance(sp,str) else sp.specie for sp in d1.site]
            d2_specie_list = [ sp.split(']_')[-1] if isinstance(sp,str) else sp.specie for sp in d2.site]
            if d1_specie_list != d2_specie_list:
                return False
            elif self.check_charge and (d1.charge != d2.charge):
                return False
        ...
        #else: # complex defect_site
        #        d1_defect_site = [PeriodicSite(s.specie, s.coords,
        #                                       d1_mod_bulk_structure.lattice,
        #                                       to_unit_cell=True) for s in d1.site]
        #        d2_defect_site = [PeriodicSite(s.specie, s.coords,
        #                                       d2_mod_bulk_structure.lattice,
        #                                       to_unit_cell=True) for s in d2.site]

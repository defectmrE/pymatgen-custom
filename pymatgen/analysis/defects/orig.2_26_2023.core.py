...
class Defect(MSONable, metaclass=ABCMeta):
    ...
    def __init__(self, structure, defect_site, charge=0., multiplicity=None):
        ...
        if isinstance(defect_site, list):
            for def_s in defect_site:
                if structure.lattice != def_s.lattice:
...
class ComplexMV(Defect):
    """
    Subclass of Defect to generate substitution/vacancy (MV-type) complex defect structure.
    """
    def __init__(self, substitution, vacancy, structure=None, defect_site=None, charge=0, multiplicity=None):
        """

        Args:
            substitution: pymatgen substitution object (identical bulk_structure to vacancy object is required)
            vacancy: pymatgen vacancy object (identical bulk_structure to substitution object is required )
            structure: pymatgen structure object
            defect_site (list of Site): [Site] for one substitution and one vacancy site
            charge (Int or float): defect charge
            multiplicity (int): multiplicity of defect within the supercell, multiplicity is assigned as the
            multiplication of the multiplicities of simple substitution and vacancy.
        """
        if not (isinstance(substitution, Substitution) and isinstance(vacancy, Vacancy)):
            raise ValueError("Incorrect substitution and vacancy object have been supplied.")

        self.substitution = substitution
        self.vacancy = vacancy
        self.structure = structure if structure else substitution.bulk_structure
        self._defect_site = defect_site if defect_site else [self.substitution.site, self.vacancy.site]
        super().__init__(structure=self.structure, defect_site=self._defect_site, charge=charge)

        self._multiplicity = multiplicity if multiplicity else self.get_multiplicity()

    def get_multiplicity(self):
        """
        Returns the multiplicity of a complex defect site within the structure (needed for concentration analysis)
        """
        return self.substitution.get_multiplicity() * self.vacancy.get_multiplicity()

    @property
    def defect_composition(self):
        poss_deflist = sorted(
            self.bulk_structure.get_sites_in_sphere(self.substitution.site.coords, 0.1, include_index=True), key=lambda x: x[1])
        defindex = poss_deflist[0][2]
    ...
    def generate_defect_structure(self, supercell=(1, 1, 1)):
        """
        Returns Substitution-Vacancy (MV-type) complex defect structure, decorated with charge
        Args:
            supercell (int, [3x1], or [[]] (3x3)): supercell integer, vector, or scaling matrix
        """
        ...
        for idx, dist in zip(np.argsort(mv_dist_list), np.sort(mv_dist_list)):
            if dist > 0:
                sub_index_in_struct = idx
                break

        if sub_index_in_struct is None:
    ...
    def name(self):
        ...
        return f"MV_{sub_name[0]}_{vac_name[0]}_mult{self.multiplicity}"

    def copy(self):
        return self.from_dict(self.as_dict())


class ComplexMI(Defect):
    """
    Subclass of Defect to generate substitution/interstitial (MI-type) complex defect structure.
    """
    def __init__(self, substitution, interstitial, structure=None, defect_site=None, charge=0, site_name='',
                 multiplicity=None):
        """
        ...
        Args:
            ...
            vacancy: pymatgen vacancy object (identical bulk_structure to substitution object is required )
            structure: pymatgen structure object
            defect_site (list of Site): [Site] for one substitution and one interstitial site
            charge (Int or float): defect charge
            multiplicity (int): multiplicity of defect within the supercell, multiplicity is assigned as the
            multiplication of the multiplicities of simple substitution and interstitial.
        """
        if not (isinstance(substitution, Substitution) and isinstance(interstitial, Interstitial)):
            raise ValueError("Incorrect substitution and interstitial object have been supplied.")

        self.substitution = substitution
        self.interstitial = interstitial
        self.structure = structure if structure else substitution.bulk_structure
        self._defect_site = defect_site if defect_site else [self.substitution.site, self.interstitial.site]
        super().__init__(structure=self.structure, defect_site=self._defect_site, charge=charge)

        self._multiplicity = multiplicity if multiplicity else self.get_multiplicity()
        self.site_name = site_name if site_name else self.interstitial.site_name

    def get_multiplicity(self):
        """
        Returns the multiplicity of a complex defect site within the structure (needed for concentration analysis)
        """
        return self.substitution.get_multiplicity() * self.interstitial.get_multiplicity()

    @property
    def defect_composition(self):
        temp_comp = self.bulk_structure.composition.as_dict()
        temp_comp[str(self.interstitial.site.specie)] -= 1
        temp_comp[str(self.substitution.site.specie)] += 1
        return Composition(temp_comp)

    def generate_defect_structure(self, supercell=(1, 1, 1)):
        """
        Returns Substitution-Interstitial (MI-type) complex defect structure, decorated with charge
        Args:
            supercell (int, [3x1], or [[]] (3x3)): supercell integer, vector, or scaling matrix
        """
        ...
        struct_for_inter_site = Structure(
            self.bulk_structure.copy().lattice,
            [self.interstitial.site.specie],
            [self.interstitial.site.frac_coords],
            to_unit_cell=True,
        )

        struct_for_sub_site.make_supercell(supercell)
        struct_for_inter_site.make_supercell(supercell)

        # these are used to construct site
        sub_site = struct_for_sub_site[0]
        inter_site_in_struct = struct_for_inter_site[0]
        ...
        symm_equiv_sub_list = [site for site in defect_structure
                               if sgop.are_symmetrically_equivalent([temp_sub_site], [site])]
        mi_dist_list = [inter_site_in_struct.distance(site) for site in symm_equiv_sub_list]

        sub_index_in_struct = None
        for idx, dist in zip(np.argsort(mi_dist_list), np.sort(mi_dist_list)):
            if dist > 0:
                sub_index_in_struct = idx
                break
        ...
        # generate interstitial first
        defect_structure.append(self.interstitial.site.specie.symbol,
                                inter_site_in_struct.coords,
                                coords_are_cartesian=True,
                                properties=None)
    ...
    def name(self):
        # TODO: find defindex
        sub_name = self.substitution.name.split("_mult")
        inter_name = self.interstitial.name.split("_mult")
        return f"MI_{sub_name[0]}_{inter_name[0]}_mult{self.multiplicity}"

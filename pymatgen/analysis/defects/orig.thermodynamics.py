...
class DefectPhaseDiagram(MSONable):
    ...
    def solve_for_fermi_energy(self, temperature, chemical_potentials, bulk_dos):
        ...
        def _get_total_q(ef):
            ...
            #print(f"{ef + fdos_vbm}:{fdos.get_doping(fermi=ef + fdos_vbm, T=temperature)}")
    ...
    def plot(self, mu_elts=None, xlim=None, ylim=None, ax_fontsize=1.3, lg_fontsize=1.,
             lg_position=None, gga_gap=None, gga_vbm=None, fermi_level = None, title=None,
             correct_int_si_label=False, saved=False):
        """
        ...
        for dfct in for_legend:
            ...
            if 'Vac' == flds[0]:
                ...
                base = '$'+flds[1]
                if correct_int_si_label:
                    if flds[3] == "mult8":
                        sub_str = '_{inter\ tetra}$'
                    elif flds[3] == "mult16":
                        sub_str = '_{inter\ hex}$'
                    else:
                        raise ValueError("Unknown interstitial types")
                else:
                    sub_str = '_{inter}$'
            elif 'MV' == flds[0]:
                flds = dfct.name.split('_')
                base = '$'+flds[2]+'_{'+flds[4]+'} '
                sub_str = 'Vac_{'+flds[6]+'}$'
            elif 'MI' == flds[0]:
                flds = dfct.name.split('_')
                base = '$' + flds[2] + '_{' + flds[4] + '} '
                if flds[8] == "mult64":
                    sub_str = flds[6]+'_{inter\ tetra}$'
                elif flds[8] == "mult128":
                    sub_str = flds[6]+'_{inter\ hex}$'
            else:
                base = dfct.name
                sub_str = ''
        ...
        if gga_gap and gga_vbm:
            plt.axvline(x=gga_gap, linestyle='--', color='r', linewidth=3) # black dashed lines for gap edges
            plt.axvline(x=gga_vbm, linestyle='--', color='r', linewidth=3)

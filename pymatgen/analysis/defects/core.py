...
class Substitution(Defect):
    ...
    def generate_defect_structure(self, supercell=(1, 1, 1)):
        ...
        #changed!
        poss_deflist = sorted(
            defect_structure.get_sites_in_sphere(defect_site.coords, 3.1, include_index=True), key=lambda x: x[1])
        defindex = poss_deflist[0][2]
    ...
    def get_multiplicity(self):
        ...
        #changed!
        poss_deflist = sorted(
            periodic_struc.get_sites_in_sphere(self.site.coords, 3, include_index=True), key=lambda x: x[1])
        if not len(poss_deflist):
...
class ComplexMV(Defect):
    def __init__(self, substitution, vacancy, structure=None, defect_site=None, charge=0, multiplicity=None):
        ...
        if self.substitution.site.lattice != self.vacancy.site.lattice:
            raise ValueError("Complex defect_site lattice must be the same.")
        ...
        #defect_site = [self.substitution.site, self.vacancy.site]
        super().__init__(structure=self.structure, defect_site=self._defect_site, charge=charge)
        #self._defect_site = defect_site
        #self.sub_site = defect_site[0]
        #self.vac_site = defect_site[1]
